	...
	?></th>
		 ...
		 for($d=0;$d<count($cal_date);$d++) { 
				...
				for($kk=0;$kk<count($flight_result);$kk++){		
					...
					$name=$flight_result[$kk]['name'];
					$airline_code=$flight_result[$kk]['fnumber'];
					$orig_date1= $flight_result[$kk]['adate1'];
					$idss= $flight_result[$kk]['rand_id'];
					$orig_date2=explode(' ',$orig_date1);
					//print_r($Total_FareAmount1);
					$orig_date3 = $orig_date2[0];
					}
					else{$_SESSION[$id][$date_id]['date_sd']=$date_sd1['cal_date'][$kk];
						$flight_id1=$flight_result[$kk]['id'];
					$Total_FareAmount1=$flight_result[$kk]['Total_FareAmount'];
					...
					//echo strtotime(date('d-m-y',strtotime($cal_date[$d]))).'<br/>'.strtotime($orig_date3);
					if(isset($Total_FareAmount1))
					{ 	
								 if(strtotime(date('d-m-y',strtotime($cal_date[$d])))==strtotime($orig_date3)){ if($d==3){
									// echo 'nbnb';?>
							<td class="style_bg_div_part_select my" onclick="showdetials(<?php echo $flight_id1;?>)">
				  ...
				  <td class="style_bg_div_part show_details my" onclick="showdetials(<?php echo $flight_id1;?>)">
